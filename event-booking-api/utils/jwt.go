...
func ValidateToken(token string) (int64, error) {
	parsedToken, err := jwt.Parse(token, func(token *jwt.Token) (interface{}, error) {
		_, ok := token.Method.(*jwt.SigningMethodHMAC) // check if the signing method is of type jwt.SigningMethodHMAC
	...
	claims, ok := parsedToken.Claims.(jwt.MapClaims) // get the claims from the token
	if !ok {
